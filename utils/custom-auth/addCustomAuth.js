...
const addSupervisorAuth = async (options) => {
  await Promise.all(
    options.locationIds.map(async (id) => {
      const location = await db.collection('locations').doc(id).get();
      if (
        location.exists &&
        location.data().supervisors.indexOf(options.uid) < 0
      ) {
        console.log(
          'Adding uID (' + options.uid + ') to location ' + 'with supervisors:',
          location.data().supervisors
        );
