...
const main = async () => {
  ...
  const emailsAsKeys = {};
  Object.entries(codes.data()).forEach(
    (entry) => (emailsAsKeys[entry[1]] = entry[0])
  );
  const uIDsAsKeys = {};
  await Promise.all(
    Object.entries(emailsAsKeys).map(async (entry) => {
      ...
      try {
        ...
        console.error(
          '[ERROR] Could not get user (' + entry[0] + ') b/c of ' + e.message
        );
      }
    })
  );
