...
const addUIDs = async (doc, appt) => {
  ...
  if (!appt.attendees[0].uid) {
    ...
    if (err) {
      console.warn(
        '[WARNING] Could not get user (' +
          appt.attendees[0].email +
          ') b/c of ' +
          err.message
      );
    } else {
  ...
  if (appt.supervisor && appt.supervisor.indexOf('@') >= 0) {
    ...
    if (err) {
      console.warn(
        '[WARNING] Could not get supervisor (' +
          appt.supervisor +
          ') b/c of ' +
          err.message
      );
      ...
      console.log(
        '[DEBUG] Changed supervisor field from email (' +
          user.email +
          ') to uID (' +
          user.uid +
          ').'
...
const restoreSubcollectionDocs = async (subcollection) => {
  ...
  console.log(
    '[INFO] Restoring ' + docs.length + ' ' + subcollection + ' documents...'
  );
  var count = 0;
  await Promise.all(
    docs.map(async (doc) => {
  ...
  console.log('[INFO] Restored ' + count + ' ' + subcollection + ' documents.');
};
