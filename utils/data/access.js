...
const addLocationsField = async () => {
  ...
  console.log(
    '[INFO] Adding the locations field to ' + users.length + ' users...'
  );
  await Promise.all(
    users.map(async (user) => {
  ...
  console.log(
    '[INFO] Added the locations field to ' + users.length + ' users.'
  );
};
...
const usersCustomAuth = async () => {
  ...
  console.log(
    '[INFO] Updating access custom auth claims for ' +
      users.length +
      ' users...'
  );
  await Promise.all(
    users.map(async (user) => {
      ...
      if (err)
        console.error(
          '[ERROR] Could not add custom auth claims b/c ' +
            'of error: ' +
            err.message
        );
      bar.tick();
  ...
  console.log(
    '[INFO] Updated access custom auth claims for ' + users.length + ' users.'
  );
};
  ...
  for (const user of users) {
    ...
    for (const access of accesses) {
      ...
      for (const name of Object.keys(profile.availability || {})) {
        ...
        if (index < 0) {
          console.warn(
            '[WARNING] No location (' + name + '), ' + 'skipping...'
          );
          continue;
        }
        ...
        if (profile.email.endsWith('example.com')) {
          ...
          console.log(
            '[DEBUG] ' + profile.name + ' (' + profile.uid + ")'s profile:",
            profile
          );
          const addToExceptions = readline.question(
            'Add ' +
              profile.name +
              ' (' +
              profile.email +
...
const propagateToSubcollections = async (collection = 'users') => {
  ...
  console.log(
    '[INFO] Updating access on documents from ' +
      collections.length +
      ' subcollections of ' +
      docs.length +
      ' ' +
  ...
  console.log(
    '[INFO] Updated access on documents from ' +
      collections.length +
      ' subcollections of ' +
      docs.length +
      ' ' +
