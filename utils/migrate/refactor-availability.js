...
const refactor = async (original, locations) => {
  ...
  Object.entries(original).forEach(([locationName, times]) => {
    ...
    if (!locationId)
      console.warn(
        '[WARNING] Could not get ID for ' +
          'location (' +
          locationName +
          '), adding it w/ an empty ID...'
      );
    Object.entries(times).forEach(([day, ts]) =>
      ts.forEach((t) =>
        res.push({
          location: {
            id: locationId,
...
const main = async () => {
  ...
  console.log(
    '[INFO] Refactoring availability on ' +
  ...
  return Promise.all(
    snapshot.docs.map(async (doc) => {
      ...
      if (err) {
        ...
        bar.update(count);
        console.error(
          '[ERROR] While refactoring profile (' + doc.id + '):',
          err.message
        );
        debugger;
      }
