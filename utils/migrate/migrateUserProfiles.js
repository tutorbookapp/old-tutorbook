...
function getAuth(profile) {
    switch (profile.type) {
        ...
        case '':
            ...
            return true;
        default:
            console.warn('[WARNING] User ' + profile.email +
                ' with type ' + profile.type + ' is not authenticated.');
            return false;
    };
};
...
function getHourAwayFrom(time) {
    ...
    try {
        ...
        console.warn('[WARNING] Error while returning timeString at index ' +
            (timeStrings.indexOf(time) + 60) + ':', e);
        return time;
    }
};
...
function getAvailability(profile) {
    ...
    console.log('[DEBUG] Parsed availability for ' + profile.email + ':', availability);
...
function getUserSubjects(profile) {
    ...
    if (!!profile.subjects && profile.subjects !== []) {
        ...
        console.log('[DEBUG] User ' + profile.email + ' already had subjects:', result);
        return result;
    }
    switch (profile.type) {
        case 'Tutor':
            result = filterSubjects(profile.proficientStudies);
            console.log('[DEBUG] User ' + profile.email + ' was a tutor, returning proficientStudies:', result);
            return result || [];
        case 'Pupil':
            result = filterSubjects(profile.neededStudies);
            console.log('[DEBUG] User ' + profile.email + ' was a pupil, returning neededStudies:', result);
            return result || [];
        default:
            console.warn('[WARNING] User ' + profile.email + ' had an invalid type, returning empty:', profile.type);
            return [];
    };
};
