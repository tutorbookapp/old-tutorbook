...
const listIndexes = async () => {
  ...
  return axios({
    method: 'get',
    url:
      'https://firestore.googleapis.com/v1beta1/projects/tutorbook-' +
...
const deleteIndexes = async (indexes) => {
  ...
  for (index of indexes) {
    ...
    await axios({
      ...
      console.log(
        '[ERROR] While deleting index (' + index.split('/').slice(-1)[0] + '):',
        err.message
      );
      debugger;
    });
    ...
    await Promise.all(
      COLLECTIONS.map(async (collection) =>
        axios({
          method: 'get',
          url:
            'https://firestore.googleapis.com/v1/projects/tutorbook-779d8/' +
          ...
          .catch((err) => {
            console.error(
              '[ERROR] While listing field index exceptions:',
              err.message
            );
            //debugger;
          })
