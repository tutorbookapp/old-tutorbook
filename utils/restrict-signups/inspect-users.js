...
const getDups = async (users, db = partitions.default) => {
  ...
  await Promise.all(
    users.map(async (user) => {
      const matching = (
        await db.collection('users').where('name', '==', user.data().name).get()
      ).docs;
...
const main = async () => {
  ...
  users.forEach((user) => {
    if (!user.data().email)
      return console.warn(
        '[WARNING] User (' + user.id + ') did not have an email.'
      );
  ...
  console.log(
    '[DEBUG] Found ' + studentDups.length + ' duplicate student ' + 'accounts.'
  );
  console.log(
    '[DEBUG] Found ' + teacherDups.length + ' duplicate teacher ' + 'accounts.'
  );
  console.log('[INFO] Found ' + dups.length + ' duplicate accounts.');
  const expected = students.concat(teachers).concat(dups);
  const others = users.filter(
    (u) => expected.map((u) => u.id).indexOf(u.id) < 0
  );
  ...
  await Promise.all(
    others.map(async (u) => {
      ...
      if (!user.location)
        return console.warn(
          '[WARNING] User (' + u.id + ') did not have a location.'
        );
  ...
  console.log(
    '[DEBUG] Found ' + gunnOthers.length + ' other accounts from ' + 'Gunn.'
  );
  console.log(
    '[DEBUG] Found ' + palyOthers.length + ' other accounts from ' + 'Paly.'
  );
  console.log(
    '[INFO] Found ' +
      locationOthers.length +
      ' other accounts ' +
      'with locations.'
  );
  const outliers = others.filter(
    (u) => locationOthers.map((u) => u.id).indexOf(u.id) < 0
  );
  console.log(
    '[INFO] Found ' +
      outliers.length +
  ...
  const paid = outliers.filter(
    (u) => u.data().payments && u.data().payments.type === 'Paid'
  );
