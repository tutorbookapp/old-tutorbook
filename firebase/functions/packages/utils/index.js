...
class Utils {
  ...
  static filterRequestUserData(user) {
    return {
      // Needed info to properly render various user headers
      name: user.name,
      email: user.email,
      uid: user.uid,
      id: user.id,
  ...
  static addDurationStrings(current, duration) {
    ...
    return f(currentHours) + ':' + f(currentMinutes) + ':' + f(currentSeconds);
  }
  ...
  static async getSupervisorForLocation(locationName, isTest = false) {
    ...
    var supervisorId;
    (
      await db.collection('locations').where('name', '==', locationName).get()
    ).forEach((doc) => (supervisorId = doc.data().supervisors[0]));
    if (!supervisorId)
      throw new Error(
        'Could not get supervisor for the ' +
          locationName +
          ' (' +
          (isTest ? 'test' : 'default') +
          ').'
      );
    return (await db.collection('users').doc(supervisorId).get()).data();
  }

  static async getUserFromPhone(phone, isTest = false) {
    const db = isTest ? partitions.test : partitions.default;
    var user;
    (await db.collection('users').where('phone', '==', phone).get()).forEach(
      (doc) => (user = doc.data())
    );
    return user;
  }
