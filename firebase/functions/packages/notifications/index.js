...
const dailyApptNotifications = async (context) => {
  ...
  return Promise.all(
    locations.map(async (doc) => {
      const config = doc.data().config;
      if (
        !config.dailyApptNotifications ||
        (!config.dailyApptNotifications.email &&
          !config.dailyApptNotifications.sms)
      )
        return;
      return axios({
        method: 'get',
        url:
          'https://us-central1-tutorbook-779d8.cloudfunctions.net/appt' +
          'Notification',
        params: {
...
const weeklyApptNotifications = async (context) => {
  ...
  return Promise.all(
    locations.map(async (doc) => {
      const config = doc.data().config;
      if (
        !config.weeklyApptNotifications ||
        (!config.weeklyApptNotifications.email &&
          !config.weeklyApptNotifications.sms)
      )
        return;
      return axios({
        method: 'get',
...
const apptNotification = (req, res) => {
  return cors(req, res, async () => {
    if (!req.query.tutor && !req.query.pupil) {
      ...
      return console.warn(
        '[WARNING] Request did not send any ' + 'notifications.'
      );
    }
    const db = req.query.test === 'true' ? partitions.test : partitions.default;
    const users = db.collection('users');
    const token = await admin.auth().verifyIdToken(req.query.token);
    if (!token.supervisor) {
      res.send('[ERROR] Invalid supervisor authentication token.');
      return console.warn(
        '[WARNING] Request did not send a valid ' +
    ...
    (
      await admin
        .firestore() // TODO: Split this query by partition
        .collectionGroup('appointments')
        .where('location.id', '==', req.query.location)
        .where('time.day', '==', upper(req.query.day))
        .get()
    ).forEach((doc) => {
      appts.push(doc.data());
    });
    await Promise.all(
      appts.map(async (appt) => {
        if (
          req.query.tutor === 'true' &&
          tutors.indexOf(appt.for.toUser.uid) < 0
        ) {
          tutors.push(appt.for.toUser.uid);
          const tutor = (await users.doc(appt.for.toUser.uid).get()).data();
          await new SMS(
            tutor,
            supervisor.name +
              ' wanted to ' +
        ...
        if (
          req.query.pupil === 'true' &&
          pupils.indexOf(appt.for.fromUser.uid) < 0
        ) {
          pupils.push(appt.for.fromUser.uid);
          const pupil = (await users.doc(appt.for.fromUser.uid).get()).data();
          await new SMS(
            pupil,
            supervisor.name +
              ' wanted to ' +
...
const userNotification = async (snap, context) => {
  ...
  if (!profile || !profile.name)
    return console.warn(
      '[WARNING] Cannot send' + ' welcome notifications to users without names.'
    );
  console.log(
    '[DEBUG] Sending ' +
      profile.name +
  ...
  await new SMS({
    recipient: profile,
    body:
      "Welcome to Tutorbook! This is how you'll receive SMS " +
      'notifications.',
    isTest: getTest(context),
    botOnSuccess: true,
    botMessage:
      'Sent ' + profile.name.split(' ')[0] + ' welcome ' + 'notifications.',
  }).send();
  console.log(
    '[DEBUG] Sent ' +
      profile.name +
...
const announcementNotification = async (snap, context) => {
  ...
  if (msg.sentBy.name === 'Operator')
    return console.log(
      '[DEBUG] Skipping ' + 'message (' + msg.message + ') sent by Operator.'
    );
  const db = getDB(context);
  const isTest = getTest(context);
  // 1) Get all users that match announcement group filters
  console.log(
    '[DEBUG] Getting users that match announcement group ' + 'filters...'
  );
  ...
  (
    ...
    const c = chat.data();
    if (c.chatters.length !== 2)
      return console.warn(
        '[WARNING] Skipping ' +
          'non-DM chat (' +
          chat.id +
          ') w/out exactly two chatters.'
      );
    const other =
      c.chatters[0].uid !== msg.sentBy.uid ? c.chatters[0] : c.chatters[1];
    supervisorDMs[other.uid] = chat.ref;
  });
  console.log(
    '[DEBUG] Got ' +
      Object.keys(supervisorDMs).length +
      ' existing DM chats with these users:',
    Object.keys(supervisorDMs)
  );
  return Promise.all(
    users.map(async (user) => {
      if (!supervisorDMs[user.uid]) {
        const chat = {
          ...
          chatters: [msg.sentBy, Utils.filterRequestUserData(user)],
          chatterUIDs: [msg.sentBy.uid, user.uid],
          chatterEmails: [msg.sentBy.email, user.email],
          location: {
            id: locRef.id,
            name: loc.name,
          },
...
const notifyAboutMessage = async (msg, recipient, isTest, botChats) => {
  ...
  if (!msg.skipEmail)
    console.warn(
      '[WARNING] Email notifications for ' + 'messages are not yet implemented.'
    );
  console.log(
    '[DEBUG] Sent ' +
      (notifications.length ? notifications.join(', ') : 'no') +
      ' message ' +
      'notifications to ' +
      recipient.name +
      ' (' +
...
const messageNotification = async (snap, context) => {
  ...
  if (getTest(context))
    return console.log(
      '[DEBUG] Skipping ' + msgString + ' sent while testing.'
    );
  if (msg.skipSMS)
    console.log('[DEBUG] Skipping SMS notifications for ' + msgString + '.');
  if (msg.skipWebpush)
    console.log('[DEBUG] Skipping webpush notifications ' + msgString + '.');
  if (msg.skipEmail)
    console.log('[DEBUG] Skipping email notifications for ' + msgString + '.');
  const db = getDB(context);
  const chatRef = db.collection('chats').doc(context.params.chat);
  const chat = (await chatRef.get()).data();
  return Promise.all(
    chat.chatterUIDs.map(async (uid) => {
      if (uid === msg.sentBy.uid)
        return console.log('[DEBUG] Skipping ' + 'notifications for sender.');
...
const rulesNotification = async (snap, context) => {
  ...
  const pupil = (await users.doc(appt.for.fromUser.uid).get()).data();
  const supervisorId = (
    await db.collection('locations').doc(context.params.location).get()
  ).data().supervisors[0];
  const supervisor = (await users.doc(supervisorId).get()).data();
  if (getTest(context))
    return console.log(
      '[DEBUG] Skipping rules email ' +
  ...
  return Promise.all(
    [tutor, pupil, supervisor].map((user) => {
...
const requestNotification = async (snap, context) => {
  ...
  console.log(
    '[DEBUG] Sent new lesson request notification to ' +
      u.name +
      ' <' +
      u.email +
      '> <' +
...
const approvedRequestNotification = async (snap, context) => {
  ...
  const summary =
    approvedBy.name +
    ' approved your lesson request. You' +
    ' now have tutoring appointments for ' +
    request.subject +
    ' with ' +
  ...
  console.log(
    '[DEBUG] Sent appt notification to ' +
      u.name +
      ' <' +
      u.email +
      '> <' +
...
const clockIn = async (snap, context) => {
  console.warn(
    '[WARNING] This notification function has not been ' + 'implemented yet.'
  );
};
...
const modifiedRequestIn = async (snap, context) => {
  ...
  const summary =
    modifiedBy.name +
    ' modified ' +
    (modifiedBy.uid === r.fromUser.uid
      ? getPronoun(modifiedBy.gender)
      : r.fromUser.name.split(' ')[0] + "'s") +
    ' lesson request to you for ' +
    r.subject +
    ' on ' +
    r.time.day +
    's at ' +
  ...
  console.log(
    '[DEBUG] Sent modified request notification to ' +
      u.name +
      ' <' +
      u.email +
      '> <' +
...
const modifiedRequestOut = async (snap, context) => {
  ...
  const summary =
    modifiedBy.name +
    ' modified your lesson request for ' +
    r.subject +
    ' on ' +
    r.time.day +
    's at ' +
...
const canceledRequestIn = async (snap, context) => {
  ...
  const summary =
    canceledBy.name +
    ' canceled ' +
    (canceledBy.uid === r.fromUser.uid
      ? getPronoun(canceledBy.gender)
      : r.fromUser.name.split(' ')[0] + "'s") +
    ' lesson request to you for ' +
    r.subject +
    ' on ' +
  ...
  console.log(
    '[DEBUG] Sent canceled request notification to ' +
      u.name +
      ' <' +
      u.email +
      '> <' +
...
const rejectedRequestOut = async (snap, context) => {
  ...
  console.log(
    '[DEBUG] Sent rejected request notification to ' +
      u.name +
      ' <' +
      u.email +
      '> <' +
...
const modifiedAppt = async (snap, context) => {
  ...
  console.log(
    '[DEBUG] Sent modified appt notification to ' +
      u.name +
      ' <' +
      u.email +
      '> <' +
...
const canceledAppt = async (snap, context) => {
  ...
  console.log(
    '[DEBUG] Sent canceled appt notification to ' +
      u.name +
      ' <' +
      u.email +
      '> <' +
