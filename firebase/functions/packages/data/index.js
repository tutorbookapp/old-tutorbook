...
class DataProxy {
  constructor(user, token, action, data) {
    ...
    ['photo', 'grade', 'gender', 'payments', 'proxy'].forEach((attr) => {
      if (!user[attr] || user[attr] === '')
        console.warn(
          '[WARNING] User did not have a valid ' +
            attr +
    ...
    global.app = {
      ...
      conciseUser: {
        ...
        photo:
          user.photo ||
          'https://tutorbook.app/app/img/' +
            (user.gender === 'Female' ? 'female.png' : 'male.png'),
        type: user.type,
        grade: user.grade || 'Sophomore',
        gender: user.gender || 'Male',
        hourlyCharge:
          user.payments && user.payments.hourlyCharge
            ? user.payments.hourlyCharge
            : 0,
  ...
  async act() {
    ...
    switch (action) {
      ...
      case 'updateLocation':
        assert(token.supervisor && token.locations.indexOf(data.id) >= 0);
      ...
      case 'newRequest':
        assert(token.uid === data.request.fromUser.uid || token.supervisor);
      ...
      case 'requestPaymentFor':
        assert(
          [data.appt.attendees[0].uid, data.appt.attendees[1].uid].indexOf(
            token.uid
          ) >= 0 &&
            user.type === 'Tutor' &&
      ...
      case 'approveRequest':
        assert(token.uid === data.request.toUser.uid || token.supervisor);
      ...
      case 'modifyRequest':
        assert(
          [data.request.fromUser.uid, data.request.toUser.uid].indexOf(
            token.uid
          ) >= 0 || token.supervisor
        );
...
class Data {
  ...
  static async newTimeRequest(request) {
    request.appt.clockIn.sentTimestamp = new Date(
      request.appt.clockIn.sentTimestamp
    );
    request.appt.clockOut.sentTimestamp = new Date(
  ...
  static async approveTimeRequest(request, id) {
    ...
    const originalRef = locationRef.collection('timeRequests').doc(id);
    const statRef = locationRef.collection('recentActions').doc(id);
    const approvedRef = locationRef.collection('approvedTimeRequests').doc(id);
  ...
  static async approvePayment(approvedPayment, id) {
    ...
    const payments = [
      db
        .collection('users')
        .doc(approvedPayment.to.uid)
        .collection('approvedPayments')
        .doc(id),
      db
        .collection('users')
        .doc(approvedPayment.from.uid)
        .collection('approvedPayments')
        .doc(id),
    ];
    const requestedPayment = db
      .collection('users')
      .doc(global.app.user.uid)
      .collection('requestedPayments')
      .doc(id);
  ...
  static async denyPayment(deniedPayment, id) {
    ...
    const payments = [
      db
        .collection('users')
        .doc(approvedPayment.appt.attendees[0].uid)
        .collection('deniedPayments')
        .doc(id),
      db
        .collection('users')
        .doc(approvedPayment.appt.attendees[1].uid)
        .collection('deniedPayments')
        .doc(id),
    ];
  ...
  static async createLocation(location, id) {
    ...
    const ref = id
      ? global.db.collection('locations').doc(id)
      : global.db.collection('locations').doc();
    if ((await ref.get()).exists)
      console.warn('[WARNING] Location (' + id + ') already existed.');
  ...
  static async instantClockIn(appt, id) {
    // Creates and approves clock in
    const db = global.db;
    const clockIn = {
    ...
    const activeAppts = [
      db
        .collection('users')
        .doc(appt.attendees[0].uid)
        .collection('activeAppointments')
        .doc(id),
      db
        .collection('users')
        .doc(appt.attendees[1].uid)
        .collection('activeAppointments')
        .doc(id),
      db
  ...
  static async approveClockIn(clockIn, id) {
    ...
    const activeAppts = [
      db
        .collection('users')
        .doc(clockIn.for.attendees[0].uid)
        .collection('activeAppointments')
        .doc(id),
      db
        .collection('users')
        .doc(clockIn.for.attendees[1].uid)
        .collection('activeAppointments')
        .doc(id),
      db
        .collection('locations')
    ...
    await ref.delete();
    await approvedClockIn.set(
      Data.combineMaps(clockIn, {
        approvedTimestamp: new Date(),
        approvedBy: global.app.conciseUser,
      })
    );
  ...
  static async rejectClockOut(clockOutData, id) {
    ...
    const clockOut = db
      .collection('locations')
      .doc(clockOutData.for.location.id)
  ...
  static async getLocationIdFromName(name) {
    ...
    if (!doc || !doc.exists)
      throw new Error('Location (' + name + ') did' + ' not exist.');
    return doc.id;
  }

  static async getLocationSupervisor(id) {
    const doc = await global.db.collection('locations').doc(id).get();
  ...
  static async approveRequest(request, id) {
    ...
    // already created).
    if (!request.location.id)
      request.location.id = await Data.getLocationIdFromName(
      ...
      ['sentTimestamp', 'approvedTimestamp'].forEach((time) => {
        if (typeof appt[key][time] === 'object') {
          appt[key][time] = new admin.firestore.Timestamp(
            appt[key][time]._seconds || appt[key][time].seconds,
            appt[key][time]._nanoseconds || appt[key][time].nanoseconds
          ).toDate();
        } else {
  ...
  static async modifyPastAppt(apptData, id) {
    ...
    ['clockIn', 'clockOut'].forEach((key) => {
      ['sent', 'approved', 'rounded'].forEach((time) => {
        time += 'Timestamp';
        console.log(
          '[DEBUG] Converting ' + key + ' ' + time + ' to ' + 'time object...',
          apptData[key][time]
        );
        if (apptData[key][time] === undefined) {
          ...
          apptData[key][time] = new admin.firestore.Timestamp(
            apptData[key][time]._seconds || apptData[key][time].seconds,
            apptData[key][time]._nanoseconds || apptData[key][time].nanoseconds
          ).toDate();
        } else {
  ...
  static trimObject(ob) {
    ...
    const UTCDate = new RegExp(
      '^(\\w{3}), (\\d{2}) (\\w{3}) (\\d{4}) ((' +
        '\\d{2}):(\\d{2}):(\\d{2})) GMT$'
    ); // See https://bit.ly/39VjTAO
      ...
      switch (typeof val) {
        ...
        case 'object':
          if (val instanceof Array) {
            // Array
            result[key] = val;
          } else if (val._seconds && val._nanoseconds) {
            // Timestamp
            console.log(
              '[DEBUG] Value (' +
                val +
          ...
          } else if (!val.getTime && !val.toDate) {
            // Map
  ...
  static async newRequest(request, payment) {
    ...
    if (payment && request.payment && request.payment.type === 'Paid') {
      switch (payment.method) {
        ...
        default:
          console.warn(
            '[WARNING] Invalid payment method (' +
              payment.method +
...
module.exports = {
  onRequest: (req, res) => {
    // Firebase Functions HTTP Request trigger
    return cors(req, res, async () => {
      ...
      global.db =
        req.query.test === 'true'
          ? admin.firestore().collection('partitions').doc('test')
          : admin.firestore().collection('partitions').doc('default');
      ...
      return data
        ...
        .then((result) => {
          ...
          if (!Stats.dataAction[req.query.action])
            return console.warn(
              '[WARNING] Data action (' +
                req.query.action +
                ') not yet' +
        ...
        .catch(async (err) => {
          ...
          if (!Stats.failedDataAction[req.query.action])
            return console.warn(
              '[WARNING] Failed data action (' +
                req.query.action +
                ') not yet tracked.'
            );
  ...
  onCall: async (data, context) => {
    // Firebase Function HTTPS Callable trigger
    throw new Error(
      "Tutorbook's onCall API is deprecated. Please use " +
