...
const getUserLocation = async (user, isTest) => {
  ...
  const getIdFromName = async (name) => {
    ...
    if (!doc)
      return console.error('[ERROR] No locations named ' + name.trim() + '.');
...
const createStat = async (user, stat, isTest, id) => {
  ...
  return locationID && id
    ...
    ? db
        ...
        .set(stat)
    : console.warn(
        '[WARNING] Could not create stat in ' + 'undefined location.'
      );
};
...
const failedDataAction = {
  newRequest: (user, data, err, isTest) => {
    return console.error(
      '[ERROR] The newRequest failedDataAction stat ' +
...
const dataAction = {
  newTimeRequest: async (user, data, res, isTest) => {
    ...
    const pupil =
      r.sentBy.uid !== a.attendees[0].uid ? a.attendees[0] : a.attendees[1];
    const stat = {
      title: 'New Time Request',
      subtitle: user.name.split(' ')[0] + ' logged service hours',
      summary:
        r.sentBy.name +
        ' logged a tutoring session with ' +
        pupil.name +
        ' from ' +
        clockIn +
        ' until ' +
  ...
  deletePastAppt: async (user, data, res, isTest) => {
    const a = data.appt;
    const r = a.for;
    const clockIn = a.clockIn.sentTimestamp.getTime
      ? a.clockIn.sentTimestamp
      : a.clockIn.sentTimestamp.toDate
      ? a.clockIn.sentTimestamp.toDate()
      : new Date(a.clockIn.sentTimestamp);
    const stat = {
  ...
  createLocation: async (user, data, res, isTest) => {
    ...
    const created = l.timestamp.getTime
      ? l.timestamp
      : l.timestamp.toDate
      ? l.timestamp.toDate()
    ...
    const stat = {
      ...
      summary:
        ...
        '. Contact +1 (650) 861-2723 for more ' +
        'information.',
      timestamp: created,
      type: 'createLocation',
    };
...
const userUpdate = async (change, context) => {
  ...
  if (!change.after.exists) {
    ...
    if (!user.name)
      return console.warn(
        '[WARNING] Could not create stat ' + 'for user without name.'
      );
    var stat = {
      ...
      summary:
        user.name +
        "'s profile data was deleted and will be " +
        'permanently removed when you dismiss this card. To restore ' +
        user.name.split(' ')[0] +
        "'s account, email help@tutorbook" +
    ...
    var stat = {
      ...
      subtitle:
        user.name.split(' ')[0] +
        ' signed up' +
        (user.type ? ' as a ' + user.type.toLowerCase() : ''),
      summary:
        user.name +
        ' is a new ' +
        ...
        "'" +
        user.name +
        "' from your home screen to view, edit, " +
    ...
    var stat = {
      ...
      summary:
        user.name +
        "'s profile was updated. To see the changes" +
        ", search '" +
        user.name +
