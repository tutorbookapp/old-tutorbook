...
const newRequestEmail = fs.readFileSync('./html/newRequest.html').toString();
const requestApprovedEmail = fs
  .readFileSync('./html/approvedRequest.html')
  .toString();
...
class Email {
  ...
  get valid() {
    if (this.user.location === 'Paly Peer Tutoring Center')
      return console.error('[ERROR] Cannot send emails to Paly users.');
    if (!this.user || !this.user.email)
      return console.error(
        '[ERROR] ' + 'Cannot send email without a valid email address.'
      );
    if (SKIP_EMAIL)
      return console.warn(
        '[WARNING] Skipping email b/c the' +
  ...
  renderDefault(data) {
    if (!data.message || !data.subject)
      return console.error('[ERROR] ' + 'Email needs a subject and a message.');
    if (!this.user.name)
      return console.error('[ERROR] User must have a ' + 'valid name.');
  ...
  async send() {
    ...
    if (!this.user || !this.user.email)
      return console.error(
        '[ERROR] ' + 'Cannot send to undefined email addresses.'
      );
    ...
    if (err)
      return console.error(
        '[ERROR] Could not verify email ' + 'transporter b/c of ' + err.message
      );
    ...
    if (error)
      return console.error(
        '[ERROR] Could not send email b/c of ' + error.message
      );
  ...
  renderNewRequest(request) {
    // Notifies tutor of a new request
    if (!this.user.name)
      return console.error('[ERROR] User needs a valid' + ' name.');
    const summary =
  ...
  renderApprovedRequest(approvedRequest) {
    // Notifies pupil about new appts
    if (!this.user.name)
      return console.error('[ERROR] User needs a valid' + ' name.');
    ...
    this.subject =
      '[Approved Request] ' +
      approvedBy.name.split(' ')[0] +
      ' approved your lesson request for ' +
      request.subject;
  ...
  renderRules(data) {
    ...
    const vars = {
      summary:
        'You now have tutoring appointments for ' +
        request.subject +
        ' with ' +
        (this.user.email !== request.toUser.email
          ? request.toUser.name.split(' ')[0]
          : request.fromUser.name.split(' ')[0]) +
        ' on ' +
        request.time.day +
  ...
  renderWelcome() {
    // Renders welcome email (custom to user type)
    if (!this.user.name)
      return console.error('[ERROR] User needs a valid' + ' name.');
    ...
    switch (this.user.type) {
      ...
      case 'Supervisor':
        ...
        this.html = supervisorEmail.replace('{ username }', this.user.name);
        break;
      default:
        this.subject += ' | Getting started';
        this.html = genericEmail.replace('{ username }', this.user.name);
        break;
    }
  }
}
