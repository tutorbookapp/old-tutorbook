...
const updateAuth = async (change, context) => {
  ...
  for (const access of (await db.collection('access').get()).docs) {
    for (const emailDomain of access.data().domains)
      if (profile.email.endsWith(emailDomain)) profile.access.push(access.id);
  ...
  if (
    ...
    return admin
      ...
      .then(() =>
        console.log(
          '[DEBUG] Added supervisor custom auth to ' +
            profile.name +
            "'s account."
        )
      )
      .catch((err) =>
        console.error(
          '[ERROR] Could not add supervisor custom auth to ' +
            profile.name +
            "'s account b/c of " +
            err.message
        )
  ...
  return admin
    ...
    .then(() =>
      console.log(
        '[DEBUG] Removed any custom auth claims from' +
          ' ' +
          profile.name +
          "'s account."
      )
    ...
    .catch((e) =>
      console.error(
        '[ERROR] Could not remove custom auth claims from ' +
          profile.name +
          "'s account b/c of " +
          e.message
      )
