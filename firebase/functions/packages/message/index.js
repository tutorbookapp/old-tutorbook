...
class Message {
  ...
  async init() {
    ...
    if (!params.to)
      params.to = await getSupervisor(params.from.location, params.isTest);
      ...
      params.from = [
        {
          ...
          location:
            params.to[0].location || params.to[0].uid
              ? (
    ...
    if (!params.sms)
      params.sms =
        params.from[0].name.split(' ')[0] + ' says: ' + params.message;
  ...
  async send() {
    ...
    if (err)
      return console.error(
        '[ERROR] Could not send message b/c of ' + err.message
      );
    console.log(
      '[DEBUG] Sending message (' +
        this.message +
        ') from ' +
        this.from.map((u) => u.name).join(', ') +
        ' to ' +
    ...
    await Promise.all(
      this.chats.map((chat) => chat.collection('messages').doc().set(this.msg))
    );
    console.log(
      '[DEBUG] Sent message (' +
        this.message +
      ...
      this.chats.map(async (chat) => {
        ...
        (
          ...
          for (var i = 0; i < this.to.length; i++) {
            if (
              this.to[i].uid !== 'operator' &&
              d.chatterUIDs.indexOf(this.to[i].uid) < 0
            )
              return;
          }
          for (var i = 0; i < this.from.length; i++) {
            if (
              this.from[i].uid !== 'operator' &&
              d.chatterUIDs.indexOf(this.from[i].uid) < 0
            )
              return;
          }
