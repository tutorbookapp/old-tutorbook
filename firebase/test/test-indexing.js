...
const { PUPIL } = require('./data.js');

const { combinations, authedApp } = require('./utils.js');
...
describe("Tutorbook's Database Indexing", async () => {
  ...
  filterCombos.forEach((filters) =>
    it('lets users filter profiles by ' + filters.join(', '), () => {
      var query = db
        .collection('users')
        .where('access', 'array-contains-any', PUPIL.access);
      filters.map(
        (filter) =>
          (query = query.where(filter, FILTERS[filter][0], FILTERS[filter][1]))
      );
      return Promise.all(
        SORTERS.map((sorter) =>
