...
const { PROJECT_ID, FIRESTORE_RULES, FUNCTIONS_URL } = require('./config.js');
const {
  PUPIL,
  TUTOR,
  SUPERVISOR,
...
describe("Tutorbook's REST API", () => {
  ...
  async function approveRequest() {
    ...
    const state = {};
    state[
      'users/' + PUPIL.uid + '/approvedRequestsOut/' + APPROVED_REQUEST_ID
    ] = APPROVED_REQUEST;
  ...
  async function approveClockIn() {
    ...
    state[
      'locations/' + LOCATION_ID + '/activeAppointments/' + ACTIVE_APPT_ID
    ] = ACTIVE_APPT;
    state[
      'users/' + TUTOR.uid + '/activeAppointments/' + ACTIVE_APPT_ID
    ] = ACTIVE_APPT;
    state[
      'users/' + PUPIL.uid + '/activeAppointments/' + ACTIVE_APPT_ID
    ] = ACTIVE_APPT;
    await data(state);
  ...
  async function clockOut() {
    ...
    const state = {};
    state[
      'locations/' + LOCATION_ID + '/clockOuts/' + CLOCK_OUT_ID
    ] = CLOCK_OUT;
    await data(state);
  ...
  async function approveClockOut() {
    ...
    state[
      'locations/' + LOCATION_ID + '/pastAppointments/' + PAST_APPT_ID
    ] = PAST_APPT;
    state[
      'users/' + TUTOR.uid + '/pastAppointments/' + PAST_APPT_ID
    ] = PAST_APPT;
    state[
      'users/' + PUPIL.uid + '/pastAppointments/' + PAST_APPT_ID
    ] = PAST_APPT;
    await data(state);
