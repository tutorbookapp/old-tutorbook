...
const cancelAuthPayment = async (snap, context) => {
    ...
    if (!authPayment.exists) return console.warn('Auth payment (' + context
        .params.payment + ') did not exist.');
    const [err, refund] = await to(stripe.refunds.create({
        charge: authPayment.data().id, // TODO: Deduct 10% service fee?
    }));
    if (err) return console.error('Could not refund auth payment (' + context
        .params.payment + ') b/c of', err);
    await authPaymentRef.delete();
    return db.collection('stripeCustomers').doc(context.params.user)
        .collection('refunds').set(refund);
};
...
const accountURL = (req, res) => {
    return cors(req, res, async () => { // Cannot use async in CORS function
        ...
        if (!doc.exists) {
            res.status(500).send('[ERROR] Account (' + req.query.id + ') did ' +
                'not exist.');
...
const initAccount = (req, res) => {
    return cors(req, res, () => {
        ...
        return axios({
            ...
            data: {
                client_secret: (req.query.test === 'true') ?
                    functions.config().stripe.test.key : functions.config()
                    .stripe.key,
                code: req.query.code,
...
const processSentPayment = async (snap, context) => {
    ...
    async function error(err) {
        await db.collection('users').doc(payment.to.uid)
            .collection('requestsIn').doc(id).delete();
        await db.collection('users').doc(payment.from.uid)
            .collection('requestsOut').doc(id).delete();
        await db.collection('users').doc(payment.from.uid)
            .collection('sentPayments').doc(id).delete();
        new Email('', payment.from, {
            subject: '[Invalid Payment] We couldn\'t process your payment ' +
                'method',
    ...
    const [err, charge] = await to(createCharge({
        payment: payment,
        capture: false,
        isTest: isTest,
    }));
    if (err) return error(err);
    await db.collection('stripeCustomers').doc(payment.from.uid)
        .collection('authPayments').doc(id).set(charge);
    // 3) Create authPayment doc for tutor 
    const authPayments = [
        db.collection('users').doc(payment.to.uid)
        .collection('authPayments').doc(id),
        db.collection('users').doc(payment.from.uid)
        .collection('authPayments').doc(id),
    ];
    ...
    return db.collection('users').doc(payment.from.uid)
        .collection('sentPayments').doc(id).delete();
};
...
const processApprovedPayment = async (snap, context) => {
    ...
    async function error(err) {
        await db.collection('users').doc(payment.from.uid)
            .collection('appointments').doc(id).delete();
        await db.collection('users').doc(payment.to.uid)
            .collection('appointments').doc(id).delete();
        new Email('', payment.from, {
            subject: '[Invalid Payment] We couldn\'t process your payment ' +
                'method',
            message: 'We could not process payment for your lesson with ' +
                payment.for.toUser.name + ', so we\'ve canceled that ' +
                'appointment. To try again, send ' + payment.for.toUser.name
                .split(' ')[0] + ' the same request using a different payment' +
                ' method.',
        });
        throw err;
    };
    ...
    if (authCharge.exists) {
        console.log('Capturing $' + payment.amount.toFixed(2) + ' (' +
            authCharge.data().id + ') for appointment (' + id + ')...');
        // TODO: Calculate amount from actual timestamps 
        const amt = new Number(payment.amount.toFixed(2)).valueOf();
        var [err, charge] = await to(stripe.charges.capture(authCharge.data().id, {
            amount: amt * 100, // In cents
        }));
    } else {
        console.log('Charging $' + payment.amount.toFixed(2) + ' for ' +
            'appointment (' + id + ')...');
        var [err, charge] = await to(createCharge({
            payment: payment,
            isTest: isTest,
            capture: true,
        }));
    ...
    const authPayments = authCharge.exists ? [
        db.collection('stripeCustomers').doc(user)
        .collection('authPayments').doc(id),
        db.collection('users').doc(payment.from.uid)
        .collection('authPayments').doc(id),
        db.collection('users').doc(payment.to.uid)
    ...
    pastPayments.push(db.collection('users').doc(payment.to.uid)
    ...
    return Promise.all(authPayments.concat(approvedPayments)
        .map((doc) => doc.delete()));
};
