...
const find = (token) => parsed.findIndex(t => {
...
md.renderer.rules.heading_open = (tokens, idx, opts, env, slf) => {
    ...
    if (token.tag === 'h2') {
        ...
        const id = match < 0 ? 'todo' : ask ? readline.question('What should ' +
                'the id be for "' + title + '"? #' + policyId + '_') : title
            .toLowerCase().split(' ').filter(s => !/^\d+\./.test(s)).join('-');
        navItems[policyId + '_' + id] = title;
        return '</div><div id="' + policyId + '_' + id + '"><div class=' +
...
md.renderer.rules.strong_open = (tokens, idx, opts, env, slf) => {
    ...
    if (tokens[idx + 1].content.indexOf('Effective day') >= 0) return '<div' +
        ' class="terms__effective-day"><div class="sp__1"></div>';
    return slf.renderToken(tokens, idx);
};

md.renderer.rules.strong_close = (tokens, idx, opts, env, slf) => {
    if (tokens.length < 3) return slf.renderToken(tokens, idx);
    if (tokens[idx - 1].content.indexOf('Effective day') >= 0) return '</' +
        'div><div class="sp__2"></div><div>';
    return slf.renderToken(tokens, idx);
};

fs.readdir('md', (e, filenames) => {
    if (e) return console.error('[ERROR] Could not read input dir b/c of', e);
    var result = '<div class="terms__main"><div id="selected-policy" class="' +
        ...
        'list">';
    var content = '</ul><div class="terms__back-link u__hidden"><div class="' +
    ...
    filenames.forEach(filename => {
        ...
        const output = '<div class="terms__article' +
            (index !== 0 ? ' u__hidden' : '') + '" data-policy="' + policyId +
            '">\n' + md.render(input) + '</div></div>';
        var menu = '<li data-policy="' + policyId + '" class="terms__policy-' +
            'nav ' + (index !== 0 ? 'collapsed' : 'selected') + '"><div class' +
            '="terms__nav-heading" data-policy="' + policyId + '">' + title +
            '</div><div class="terms__nav-subheadings"><div class="sp__1"></' +
            'div><ul>';
        Object.entries(navItems).forEach(([id, title]) => menu += '<li><a ' +
            'href="#' + id + '" data-scroll="true">' + title + '</a></li>');
        menu += '</ul></div></li>';
        result += menu;
        content += output;
        fs.writeFileSync('html/' + policyId + '.html', output);
        fs.writeFileSync('html/' + policyId + '-menu.html', menu);
        console.log('[DEBUG] Converted ' + filename + ' to and generated menu' +
            ' list in HTML.');
        index++;
    });
    result += content;
    result += '</div></div><div class="sp__2"></div></div><div class="sp__9">' +
