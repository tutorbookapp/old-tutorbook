...
class Event {
  ...
  constructor(appt, id, colors, type, index = 0) {
    ...
    this.type = type;
    this.index = index;
    this.viewDialog = new ViewApptDialog(Utils.filterApptData(this), this.id);
    this.editDialog = new EditApptDialog(Utils.filterApptData(this), this.id);
    this.actions = {
      ...
      Cancel: () => {
        return new ConfirmationDialog(
          'Cancel Appointment?',
          'Cancel tutoring sessions between ' +
            this.attendees[0].name +
            ' and ' +
            this.attendees[1].name +
            ' for ' +
            this.for.subject +
            ' at ' +
            this.time.from +
          ...
          async () => {
            ...
            $(this.el).hide();
            const [e, r] = await to(
              Data.cancelAppt(Utils.filterApptData(this), this.id)
            );
            if (e) {
              $(this.el).show();
  ...
  renderSelf() {
    ...
    const subtitle =
      this.for.subject +
      ((window.app.data.periods[this.time.day] || []).indexOf(this.time.from) <
      0
        ? ' at '
        : ' during ') +
      this.time.from;
    this.el = $(
      this.render.template('card-event', {
        title: title,
        subtitle: subtitle,
        id: this.id,
        type: this.type,
...
class Appt extends Event {
  ...
  async clockIn() {
    ...
    const [e, r] = await to(
      Data.instantClockIn(Utils.filterApptData(this), this.id)
    );
...
class ActiveAppt extends Event {
  constructor(appt, id, colors, index) {
    ...
    this.viewDialog = new ViewActiveApptDialog(
      Utils.filterActiveApptData(this),
      this.id
    );
    this.color = '#B00020';
    this.renderSelf();
  ...
  renderSelf() {
    ...
    $(this.el)
      ...
      .end()
      .find('.mdc-menu #Cancel')
      .remove();
    var time = new Date().getTime() / 1000 - this.clockIn.sentTimestamp.seconds;
    var total =
      (new Date('1/1/2019 ' + this.time.to).getTime() -
        new Date('1/1/2019 ' + this.time.from).getTime()) /
      1000;
  ...
  async clockOut() {
    window.app.snackbar.view(
      'Clocking out for ' + this.for.toUser.name + '...'
    );
    const [e, r] = await to(
      Data.instantClockOut(Utils.filterApptData(this), this.id)
    );
  ...
  async viewAppts() {
    const recycler = {
      display: (doc, type, index) => {
        ...
        $(this.main)
          .find('#' + this[type][index][doc.id].time.day + ' .schedule-list')
          .append(this[type][index][doc.id].el);
