...
import { EditAvailabilityDialog } from '@tutorbook/dialogs';
...
export class FilterDialog {
  ...
  renderSelf() {
    ...
    const r = (query, options, addAny = true) =>
  ...
  clearFilters(filters = this.filters) {
    ...
    for (var filter in filters) {
      if (
        ...
        result[filter] =
          filter === 'availability'
            ? this.getAvailabilityString(filters[filter])
            : filters[filter];
      } else {
        result[filter] = '';
      }
  ...
  renderInputAvailability() {
    ...
    $(this.el)
      .find('#page-availability #ok-button')[0]
      .addEventListener('click', () => {
        if (!this.availabilityDialog.valid) return;
        this.filters.availability = this.availabilityDialog.val;
        this.page('page-all');
      });
    return (this.availabilityDialog.main = $(this.availabilityDialog.main).find(
      '.mdc-dialog__content'
    )[0]);
  }
}
...
export class NewGroupDialog extends FilterDialog {
  constructor(options = {}) {
    ...
    this.name =
      options.name ||
      window.app.location.name + ' Group #' + (options.groupNum || 1);
  ...
  renderSelf() {
    super.renderSelf();
    const description =
      'Filter users to create a new announcement group.' +
    ...
    const nameEl = $(this.render.wrapper())
      .append(nameTextFieldEl)
      .addClass('text-field-wrapper');
    $(this.el)
      .find('#show-page-availability span')
      .text('Any Appointments')
      .end()
      .find('#page-availability h2')
      .text('Appointments')
      .end()
      .find('#page-all .mdc-dialog__title')
      .text('New Group')
      .end()
      .find('[data-mdc-dialog-action="accept"]')
      .text('Create')
      .end()
      .find('#page-all .mdc-dialog__content')
      .prepend(nameEl)
      .prepend(description)
      .end()
