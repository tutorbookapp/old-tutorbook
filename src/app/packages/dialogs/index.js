...
export class SubjectSelectDialog {
      ...
      $(sel)
        ...
        .each(function () {
          ...
          this.addEventListener('click', (event) => {
            ...
            const category = $(this).parent().parent().attr('id').split('-')[0];
            const index = Data[category + 'Subjects'].indexOf(val);
            if (
              that.shiftKeyPressed &&
              that.lastClickedCategory === category &&
              that.lastClickedIndex !== index
            ) {
              const start =
                that.lastClickedIndex > index ? index : that.lastClickedIndex;
              const end =
                that.lastClickedIndex < index ? index : that.lastClickedIndex;
              const selected = Data[category + 'Subjects'].slice(
                start,
                end + 1
              );
              selected.forEach((v) => {
  ...
  renderSelf() {
    ...
    const l = (q, d) =>
      Utils.replaceElement($(this.main).find(q)[0], this.renderList(d));
...
export class EditSubjectsDialog extends SubjectSelectDialog {
  ...
  save() {
    ...
    const a = (e, el) =>
      $(this.render.splitListItem(e, el)).insertBefore(
...
export class EditAvailabilityDialog {
  ...
  manage() {
    ...
    this.dialog.listen('MDCDialog:closing', (event) => {
      if (event.detail.action === 'ok') {
        $(this.input)
          .find('input')
          .val(Utils.getAvailabilityString(this.val))
          .focus();
  ...
  refreshTimes() {
    ...
    if (!location)
      return console.warn(
        '[WARNING] Cannot refresh days and' + ' times w/out location data.'
      );
    ...
    const timeStrings = times.map((t) =>
      t.open !== t.close ? t.open + ' to ' + t.close : t.open
    );
    const that = this;
    ...
    if (times.length === 1) {
      ...
      return window.app.snackbar.view(
        location.name + ' does not have ' + 'any open hours.'
      );
    ...
    function r(q, el, action, id = 'timeslotSelect') {
      // Replaces select,
    ...
    r('#Time', that.render.select('Time', that.val.time, timeStrings), (s) => {
      if (s.value.split(' to ').length > 1) {
  ...
  refreshDaysAndTimes() {
    ...
    if (!location)
      return console.warn(
        '[WARNING] Cannot refresh days and' + ' times w/out location.'
      );
    var times = this.val.day
      ? this.utils.getLocationTimeWindowsByDay(this.val.day, location.hours)
      : this.utils.getLocationTimeWindows(location.hours);
    var timeStrings = times.map((t) =>
      t.open !== t.close ? t.open + ' to ' + t.close : t.open
    );
  ...
  renderSelf() {
    ...
    $(this.main)
      ...
      .removeAttr('data-' + 'mdc-dialog-action');
    ...
    addS(
      ...
      Utils.concatArr(
        [v.location || ''],
        window.location.name === 'Any'
          ? d.locationNames.concat(['Custom'])
          : d.locationNames
      )
    );
...
export class ViewRequestDialog {
  ...
  async renderSelf() {
    ...
    if (window.app.user.type === 'Supervisor') {
      ...
      addD(
        'To' +
          (request.toUser.type ? ' ' + request.toUser.type.toLowerCase() : '')
      );
    ...
    const header = this.render.header('header-action', {
      ...
      approve: async () => {
        ...
        const [err, res] = await to(Data.approveRequest(this.request, this.id));
        if (err)
          return window.app.snackbar.view('Could not approve ' + 'request.');
...
export class EditHourDialog {
  ...
  err(t, msg) {
    ...
    $(t.root_)
      .parent()
      .find('.mdc-text-field-helper-line')
      .show()
      .end()
      .parent()
      .addClass('err-input-list-item--errored');
    return false;
  }
  ...
  validate(t) {
    ...
    $(t.root_)
      ...
      .end()
      .parent()
      .removeClass('err-input-list-item--errored');
    return t.value;
  }
  ...
  updateOpenTime(t = this.openTextField, update = true) {
    ...
    if (ind < 0 && periodsInd > periods.indexOf(this.val.close))
      return this.err(t, "Opening period can't be after closing period.");
    if (!periods.length || periodsInd < 0) {
      if (ind < 0)
        return this.err(
          t,
          'Time is formatted incorrectly or' +
            " isn't on " +
            this.val.day +
            "'s bell schedule."
        );
      if (ind > this.data.timeStrings.indexOf(this.val.close))
        return this.err(t, "Opening time can't be after closing time.");
    }
  ...
  updateCloseTime(t = this.closeTextField, update = true) {
    ...
    const periodsInd = periods.indexOf(t.value);
    const ind = this.data.timeStrings.indexOf(t.value);
    if (ind < 0 && periodsInd < periods.indexOf(this.val.open))
      return this.err(t, "Closing period can't be before opening period.");
    if (!periods.length || periodsInd < 0) {
      if (ind < 0)
        return this.err(
          t,
      ...
      if (ind < this.data.timeStrings.indexOf(this.val.open))
        return this.err(t, "Closing time can't " + 'be before opening time.');
    }
...
export class EditLocationDialog {
  ...
  renderSelf() {
      ...
      cancel: () => {
        if (this.changed)
          return new ConfirmationDialog(
            'Discard ' + 'Changes?',
            'Are you sure that you want to discard your ' +
              'changes to the ' +
              this.name +
    ...
    const addS = (l, v = '', d = []) =>
      add(this.render.selectItem(l, v, Utils.concatArr(d, [v])));
     ...
     */
    add(
      this.render.template('delete-user-input', {
        label: 'Delete Location',
        delete: () =>
          new ConfirmationDialog(
            'Delete Location?',
            'You are' +
              ' about to permanently delete all ' +
              this.name +
              ' data. ' +
              'This action cannot be undone. Please ensure to check with ' +
              'your fellow supervisors before continuing.',
            async () => {
              window.app.nav.back();
              window.app.snackbar.view('Deleting location...');
              const [err, res] = await to(Data.deleteLocation(this.id));
              if (err)
                return window.app.snackbar.view(
  ...
  async save() {
    ...
    const [err, res] = await to(
      Data.updateLocation(Utils.filterLocationData(this), this.id)
    );
  ...
  get changed() {
    ...
    for (var [key, val] of Object.entries(this.location)) {
      if (
        typeof val === 'object'
          ? !Utils.identicalMaps(this[key], val)
          : this[key] !== val
      )
        return true;
    }
  ...
  addHourInputs() {
    const add = (t = '') =>
      $(this.main).append(this.render.textFieldItem('Open', t));
  ...
  addSupervisorInput() {
    ...
    $(el).insertAfter($(this.main).find('[id="Supervisor"]').last().parent());
  }
  ...
  addSupervisorInputs() {
    const add = (e, el) =>
      $(this.main).append(this.render.splitListItem(e, el));
...
export class EditRequestDialog {
  ...
  async renderSelf(profile) {
    ...
    const timeslots =
      request.time.day && request.location.name
    ...
    const timeslot =
      request.time.from === request.time.to
        ? request.time.from || ''
        : request.time.from + ' to ' + request.time.to;
    ...
    if (locations.length < 1 && days.length < 1 && timeslots.length < 1) {
      window.app.snackbar.view(
        user.name + ' does not have any other availability.'
      );
    }
  ...
  manage() {
    ...
    locationSelect.listen('MDCSelect:change', function () {
      if (locationSelect.value === 'Custom') {
        ...
        request.location.id =
          window.app.data.locationsByName[locationSelect.value]; // Only init data once
  ...
  refreshDayAndTimeSelects(request, a) {
    ...
    const that = this;
    const days = Utils.getUserAvailableDaysForLocation(
      a,
      request.location.name
    );
    ...
    if (days.length < 1 && timeslots.length < 1)
      return window.app.snackbar.view(
        request.toUser.name +
          ' does not have any availability at ' +
          'the ' +
          request.location.name +
          '.'
      );
    ...
    const oldTimeslotEl = document
      .querySelector('main .dialog-input')
      .querySelector('#Time');
  ...
  refreshTimeSelects(request, a) {
    ...
    if (timeslots.length < 1)
      return window.app.snackbar.view(
        request.toUser.name +
          ' does not have any availability on ' +
          request.day +
          's.'
      );
...
export class NewRequestDialog extends EditRequestDialog {
  ...
  constructor(subject, user) {
    ...
    if (locations.length < 1 && days.length < 1 && timeslots.length < 1)
      return window.app.snackbar.view(
        user.name + ' does not have any ' + 'availability.'
      );
  ...
  async sendRequest() {
    ...
    window.app.snackbar.view(
      ...
      async () => {
        ...
        if (err)
          return window.app.snackbar.view(
            'Could not cancel ' + 'request. Go to your dashboard to try again.'
          );
        window.app.snackbar.view(
          'Canceled request to ' + this.request.toUser.email + '.'
        );
      }
    );
  }
...
export class PaidRequestDialog extends NewRequestDialog {
  constructor(subject, user) {
    ...
    if (user.payments.type !== 'Paid') {
      console.warn(
        '[WARNING] PaidRequestDialog was passed a user that ' +
  ...
  sendRequest() {
    if (!this.payment.transaction)
      return window.app.snackbar.view('Please add a valid payment method.');
  ...
  managePayments() {
    ...
    if (!window.app.onMobile) {
      const descriptionEl = $(this.main).find('[id="Authorize payment"]')[0];
...
export class StripeRequestDialog extends PaidRequestDialog {
  ...
  async sendRequest() {
    ...
    if (res.error)
      return $(this.main).find('#Method')[0].scrollIntoView({
        behavior: 'smooth',
      });
  ...
  managePayments() {
    ...
    function showErr(error) {
      ...
      $(methodEl).find('.mdc-text-field').addClass('mdc-text-field--invalid');
      $(methodEl)
    ...
    const style = {
      base: {
        // Comes from MDCTextField styling
        fontSize: '16px',
...
export class ViewApptDialog extends ViewRequestDialog {
  ...
  async renderSelf() {
    ...
    if (window.app.user.type === 'Supervisor') {
      $(this.main)
        .find(
          '[id="From' +
            (this.request.fromUser.type
              ? ' ' + this.request.fromUser.type.toLowerCase()
              : '') +
            '"]'
        )
        .remove();
      $(this.main)
        .find(
          '[id="To' +
            (this.request.toUser.type
              ? ' ' + this.request.toUser.type.toLowerCase()
              : '') +
            '"] h4'
        )
        .text('Attendees');
  ...
  manage() {
    ...
    listen(paymentBtn, async () => {
      ...
      const [err, res] = await to(Data.requestPaymentFor(this.appt, this.id));
      if (err)
        return window.app.snackbar.view(
          'Could not send ' +
  ...
  async clockIn() {
    ...
    if (window.app.user.type === 'Supervisor') {
      window.app.snackbar.view(
        'Clocking in for ' + this.appt.for.toUser.name.split(' ')[0] + '...'
      );
      ...
      window.app.snackbar.view(
        'Clocked in at ' +
          new Date(r.clockIn.sentTimestamp).toLocaleTimeString() +
          '.'
      );
    } else {
      ...
      window.app.snackbar.view('Sending request...');
      const [err, res] = await to(Data.clockIn(this.appt, this.id, proof));
      if (err) {
        reset();
        return window.app.snackbar.view(
          'Could not send clock-in ' + 'request.'
        );
      }
      window.app.snackbar.view(
        'Sent clock-in request to ' + res.recipient.name + '.'
      );
  ...
  static listen(approvedRef, rejectedRef, name, reset = () => {}) {
    ...
    Data.listen(
      ...
      (doc) => {
        if (doc.exists)
          return window.app.snackbar.view(
            Utils.caps(name.toLowerCase()) +
              ' request ' +
              'approved by ' +
              doc.data().approvedBy.name +
              '.'
          );
      },
      (err) =>
        console.error(
          '[ERROR] Could not listen for approved ' +
            name.toLowerCase() +
            ' b/c of ',
          err
        )
    ...
    Data.listen(
      ...
      (err) =>
        console.error(
          '[ERROR] Could not listen for rejected ' +
            name.toLowerCase() +
            ' b/c of ',
          err
        )
  ...
  async clockOut() {
    ...
    if (window.app.user.type === 'Supervisor') {
      window.app.snackbar.view(
        'Clocking out for ' + this.appt.for.toUser.name.split(' ')[0] + '...'
      );
      ...
      window.app.snackbar.view(
        'Clocked out at ' +
          new Date(r.clockOut.sentTimestamp).toLocaleTimeString() +
          '.'
      );
    } else {
      const [canceled, proof] = await to(new CaptureProofDialog().view());
      ...
      window.app.snackbar.view('Sending request...');
      const [err, res] = await to(Data.clockOut(this.appt, this.id, proof));
      if (err) {
        reset();
        return window.app.snackbar.view(
          'Could not send clock-out ' + 'request.'
        );
      }
      window.app.snackbar.view(
        'Sent clock-out request to ' + res.recipient.name + '.'
      );
  ...
  update() {
    ...
    totalTimeDisplay.value =
      totalHours + ':' + totalMinutes + ':' + totalSeconds + '.' + totalMilli;
  }
}
...
export class EditApptDialog extends EditRequestDialog {
  ...
  async renderSelf() {
    ...
    if (window.app.user.type === 'Supervisor') {
      $(this.main)
        .find('[id="From ' + this.request.fromUser.type.toLowerCase() + '"]')
        .remove();
      $(this.main)
        .find('[id="To ' + this.request.toUser.type.toLowerCase() + '"] h4')
        .text('Attendees');
    }
    this.header = this.render.header('header-action', {
  ...
  async modifyRequest() {
    ...
    if (err)
      return window.app.snackbar.view('Could not modify ' + 'appointment.');
...
export class NewPastApptDialog extends EditApptDialog {
  ...
  renderSelf() {
    this.header = this.render.header('header-action', {
      ...
      ok: async () => {
        ...
        if (err)
          return window.app.snackbar.view(
            'Could not create ' + 'past appointment.'
          );
      ...
      el.addEventListener('click', () => {
        ...
        window.setTimeout(() => {
          ...
          $(this.main)
            .find('#' + opp + ' input')
            .click();
          if (opp !== 'Location') this[opp.toLowerCase() + 'TextField'].focus();
        }, 50);
      });
      return el;
    };
    ...
    const searchPupils = async (textFieldItem) => {
      ...
      res.hits.forEach((hit) => {
        try {
          $(textFieldItem).find('#results').append(renderHit(hit, 'Pupil'));
        } catch (e) {
          console.warn(
    ...
    const searchTutors = async (textFieldItem) => {
      ...
      res.hits.forEach((hit) => {
        try {
          $(textFieldItem).find('#results').append(renderHit(hit, 'Tutor'));
        } catch (e) {
          console.warn(
            '[ERROR] Could not render hit (' + hit.objectID + ') b/c of',
            e
  ...
  updateClockingTimes() {
    ...
    const editTime = async (t) => {
      const request =
        t.root_.id === 'Clock-in' ? this.appt.clockIn : this.appt.clockOut;
  ...
  refreshData() {
    ...
    this.availability = Utils.combineAvailability(
      this.request.fromUser.availability,
      this.request.toUser.availability
    );
    ...
    this.subjects = Utils.concatArr(
      this.request.fromUser.subjects,
      this.request.toUser.subjects
    );
    ...
    this.req = this.req.filter(
      (r) => ['Location', 'Subject'].indexOf(r.id) < 0
    );
    this.req.push({
    ...
    this.req.push({
      ...
      id: 'Location',
      valid: () => window.app.data.locationsByName[this.locationSelect.value],
    });
    this.refreshDayAndTimeSelects(this.request, this.availability);
  }
  ...
  manage() {
    ...
    $(this.header)
      ...
      .end()
      .find('.mdc-select,.search-results li')
      .each(function () {
        MDCRipple.attachTo(this);
      });
    ...
    this.messageTextField = t(
      '#Message',
      (t) => (this.request.message = t.value),
      'textarea'
    );
    ...
    this.req.push({
      ...
      id: 'Date',
      valid: () =>
        new Date(this.dateTextField.value).toString() !== 'Invalid Date',
    });
    this.req.push({
...
export class ViewPastApptDialog extends ViewApptDialog {
      ...
      delete: () => {
        return new ConfirmationDialog(
          ...
          'Are you sure you want to permanently delete this ' +
            'past appointment between ' +
            this.appt.attendees[0].name +
            ' and ' +
            this.appt.attendees[1].name +
            '? This action ' +
            'cannot be undone.',
          async () => {
            ...
            if (err)
              return window.app.snackbar.view(
                'Could not ' + 'delete past appointment.'
              );
  ...
  manage() {
    ...
    const editClockingTime = async (id) => {
      if (this.appt.clockIn.sentTimestamp.toDate)
        this.appt.clockIn.sentTimestamp = this.appt.clockIn.sentTimestamp.toDate();
      if (this.appt.clockOut.sentTimestamp.toDate)
        this.appt.clockOut.sentTimestamp = this.appt.clockOut.sentTimestamp.toDate();
      const date =
        id === 'Clock-in'
          ? this.appt.clockIn.sentTimestamp
          : this.appt.clockOut.sentTimestamp;
      ...
      const [err, res] = await to(Data.modifyPastAppt(this.appt, this.id));
      if (err)
        return window.app.snackbar.view(
          'Could not update past ' + 'appointment.'
        );
      window.app.snackbar.view('Updated past appointment.');
    };
    $(this.main)
      .find('[id="Clock-in"] input')
      .removeAttr('disabled')
      .focusout(async () => {
        editClockingTime('Clock-in', this.appt.clockIn.sentTimestamp);
      })
      .end()
      .find('[id="Clock-out"] input')
      .removeAttr('disabled')
      .focusout(async () => {
...
export class ViewTimeRequestDialog extends ViewPastApptDialog {
  ...
  async updateRenderAgain() {
    await this.rendering;
    $(this.render.listDivider('Proof')).insertBefore($(this.main).find('#At'));
