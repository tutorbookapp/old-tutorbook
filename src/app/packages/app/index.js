...
// App packages
import { Dashboard, SupervisorDashboard } from '@tutorbook/dashboard';
...
import { Schedule, SupervisorSchedule } from '@tutorbook/schedule';
import { Chats, SupervisorChats } from '@tutorbook/chats';
import {
  Login,
  GoToWebsitePrompt,
  GoToRootPrompt,
...
export default class Tutorbook {
  ...
  constructor() {
     ...
     */
    this.functionsURL =
      'https://us-central1-tutorbook-779d8.cloudfunctio' + 'ns.net/';
  ...
  async initURLParams() {
    for (const [key, val] of new URLSearchParams(window.location.search)) {
      ...
      if (err) {
        ...
        this.snackbar.view(
          'Could not connect payments ' + 'account.',
          'Retry',
          () => (window.location = this.payments.setupURL)
        );
      } else {
        this.snackbar.view('Connected payments account.', 'View', () =>
    ...
    return Data.listen(
      ...
      (websiteConfigsSnapshot) => {
        ...
        if (websiteConfigs.length === 1) {
          ...
          console.warn(
            '[WARNING] There was more than one website ' +
              'config for ' +
              window.location +
              ', using first...'
          );
          ...
          console.warn(
            '[WARNING] There was no website config for ' +
              window.location +
      ...
      (error) => {
        console.error(
          '[ERROR] Could not get website configurations, ' +
            'acting as if root partition...',
          error
        );
      },
      {
  ...
  async checkConfigCompliance(user, profile) {
    ...
    if (this.checked) {
      ...
      // If the website doesn't have a config (i.e. root partition):
      this.checked = true;
      ...
      for (const access of accesses) {
        ...
        for (const emailDomain of access.data().domains)
          if (user.email.endsWith(emailDomain)) return showPromptScreen(access);
        for (const email of access.data().exceptions)
          if (user.email === email) return showPromptScreen(access);
      }
  ...
  logJobPost() {
    ...
    console.log('===============================================');
    console.log(
      "Taking a look under the hood? We'd love to have you on " +
