...
import { Profile, PaidTutorProfile, TutorProfile } from '@tutorbook/profile';
import { NotificationDialog } from '@tutorbook/dialogs';
...
export default class Payments {
  ...
  async initStripe() {
    ...
    const url = window.app.test
      ? 'http://localhost:5005/app'
      : 'https://' +
        (window.app.location.name === 'Gunn Academic Center'
          ? 'gunn.'
          : window.app.location.name === 'Paly Peer Tutoring Center'
          ? 'paly.'
          : window.app.location.name === 'JLS Library'
          ? 'jls.'
          : '') +
        'tutorbook.app/app';
    const client = window.app.test
      ? 'ca_Fv3pF2bbTTRhgS5Buzieaikie2hUOR3C'
      : 'ca_Fv3pVzsbrh5FfoBxIRnO8dsbmD6XNtHX';
    this.setupURL =
      'https://connect.stripe.com/express/oauth/' +
      ...
      window.app.user.name.split(' ')[0] +
      '&stripe_user[' +
      'last_name]=' +
      window.app.user.name.split(' ')[1] +
      ...
      'Tutorbook.&stripe_user[url]=' +
      url +
      '/users/' +
      window.app.user.uid;
    const doc = await window.app.db
      ...
      return axios({
        ...
        .then((res) => {
          if (typeof res.data === 'string' && res.data.indexOf('ERROR') > 0) {
            console.error(
              '[ERROR] On server while fetching new ' +
                'Stripe Connect Account url:',
              res.data
            );
        ...
        .catch((err) => {
          console.error(
            '[ERROR] While fetching new Stripe Connect ' + 'Account url:',
            err
          );
  ...
  manage() {
    ...
    function disable(yes) {
      ...
      hours.disabled = yes;
      $(main).find('[id="Current balance"] input').attr('disabled', 'disabled');
      $(main)
        .find('[id="Total hours worked"] input')
        .attr('disabled', 'disabled');
    }
  ...
  renderSelf() {
    ...
    if (window.app.user.type === 'Tutor') {
      $(this.render.listDivider('My business')).insertBefore(
        $(this.main).find('#settings')
      );
    ...
    $(this.render.listDivider('Transaction history')).insertBefore(
      $(this.main).find('#history')
    );
  }
  ...
  renderBusiness() {
    ...
    view.appendChild(
      this.render.selectItem(
        'Type',
        window.app.user.payments.type || '',
        this.data.payments.types
      )
    );
    view.appendChild(
      this.render.splitListItem(
        this.render.select(
          'Hourly charge',
          window.app.user.payments.hourlyChargeString,
          this.data.payments.hourlyChargeStrings
        ),
        this.render.textField(
          'Current balance',
          window.app.user.payments.currentBalanceString
        )
      )
    );
    view.appendChild(
...
Payments.renderPastPayment = function (doc) {
  ...
  const listItem = window.app.render.template('transaction-list-item', {
    ...
    timestamp: time,
    go_to_transaction: () => {
      that.viewPastApptDialog(
        that.combineMaps(payment.appt, {
          id: doc.id,
        })
      );
    },
...
Payments.renderDeniedPayment = function (doc) {
  ...
  if (window.app.user.email === payment.for.from.email) {
    ...
    var subtitle =
      payment.for.from.name +
      ' denied a payment.for.to you due to ' +
      'an unsatisfactory experience. See your reviews for more detail.';
  } else {
    var subtitle =
      payment.for.from.name +
      ' denied a payment.for.to ' +
      payment.for.to.name +
      '. We are currently processing this refund.' +
...
Payments.renderApprovedPayment = function (doc) {
  ...
  if (window.app.user.email === payment.for.from.email) {
    ...
    var subtitle =
      payment.for.from.name +
      ' approved a payment to you.' +
      ' We are currently processing this payment. Your funds should be ' +
      'available shorty.';
  } else {
    var subtitle =
      payment.for.from.name +
      ' approved a payment.for.to ' +
      payment.for.to.name +
      '. We are currently processing this payment.' +
      ' Funds should be transferred shortly.';
  }
  const meta_title = '$' + payment.for.amount.toFixed(2);
  const meta_subtitle = getDayAndDateString(time);
...
Payments.renderInvalidPayment = function (doc) {
  ...
  if (window.app.user.email === payment.from.email) {
    ...
    var subtitle =
      ...
      "'s payment to " +
      payment.to.name +
      ' was invalid. Their appointment(s) will be ' +
      'canceled in 24 hours unless ' +
      payment.from.name.split(' ')[0] +
      ' adds a valid payment method.';
  }
  const meta_title = '$' + payment.amount.toFixed(2);
...
Payments.renderAuthPayment = function (doc) {
  ...
  if (window.app.user.email === payment.from.email) {
    var subtitle =
      ...
      Utils.getPronoun(payment.to.gender) +
      ' lesson.';
  } else if (window.app.user.email === payment.to.email) {
    var subtitle =
      payment.from.name +
      ' authorized a payment to you.' +
