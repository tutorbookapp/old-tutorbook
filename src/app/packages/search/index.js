...
import { FilterDialog } from '@tutorbook/filters';
...
export class SearchHeader {
      ...
      Utils.combineMaps(
        {
          ...
          wordmark: false,
          logo: false,
          placeholder: window.app.onMobile
            ? 'Search users'
            : 'Search users' + ' by name, subject, availability, and more',
        },
        options
      )
    );
  ...
  showResults() {
    ...
    $(this.el).find('.search-box').addClass('search-box--elevated');
  }

  hideResults() {
    $(this.el).find('.search-results').hide();
    $(this.el).find('.search-box').removeClass('search-box--elevated');
  }
  ...
  static renderHit(hit, render) {
    // TODO: Remove code duplication from Search()
    var el;
    ...
    listItemData.hrs = async () => {
      ...
      const [err, res] = await to(
        Data.getServiceHoursLog({
          uid: profile.uid,
        })
      );
      if (err)
        return window.app.snackbar.view(
          'Could not generate ' + 'service hour log.'
        );
      window.app.snackbar.view(
        'Generated service hour log.',
        'view',
        () => window.open(res),
        true,
        -1
...
export class Search {
  constructor() {
    ...
    this.recycler = {
      ...
      empty: () => {
        return $(this.main).find('#results').empty().append(this.emptySearch);
      },
    };
    this.renderSelf();
  }
  ...
  initDescription() {
    ...
    if (this.filters.type !== 'Any') {
      ...
      if (this.filters.grade === 'Any') {
        this.filterDescription +=
          this.filters.price === 'Any' ? 'all users' : 'users';
  ...
  async reViewUser(id) {
    if (!this.users[id]) this.users[id] = new User(await Data.getUser(id));
  ...
  viewResults() {
    ...
    this.initDescription();
    $(this.header)
      .find('[data-fir-content="filter_description"]')
      .text(this.filterDescription);
    this.emptyResults();
    window.app.listeners.push(
      this.getUsers().onSnapshot({
        error: (err) =>
          console.error(
            '[ERROR] Could not show search ' + 'results b/c of ',
            err
          ),
        next: (snapshot) => {
          if (!snapshot.size) {
