      ...
      action: () => {
        return new ConfirmationDialog(
          ...
          async () => {
            ...
            const [err, res] = await to(Data.cancelAppt(doc.data(), doc.id));
            if (err) {
              $(this.el).show();
              window.app.schedule.refresh();
            ...
            window.app.snackbar.view(
              'Canceled appointment with ' + this.other.email + '.'
            );
          }
        ).view();
      },
...
export class CanceledAppt extends Event {
  constructor(doc) {
    super(doc);
    this.other = Utils.getOtherUser(
      this.for.attendees[0],
      this.for.attendees[1]
    );
    this.title = 'Canceled Appointment with ' + this.canceledBy.name;
    this.subtitle =
      this.canceledBy.name +
      ' canceled this upcoming ' +
...
export class SupervisorCanceledAppt extends Event {
  constructor(doc) {
    super(doc);
    this.title =
      'Canceled Appointment between ' +
      this.for.attendees[0].name +
      ' and ' +
      this.for.attendees[1].name;
    this.subtitle =
      this.canceledBy.name +
      ' canceled this upcoming ' +
      'appointment. Please ensure to address these changes.';
...
export class ActiveAppt extends Event {
  constructor(doc) {
    ...
    this.subtitle =
      'Tutoring session right now for ' +
      this.for.subject +
      ' at the ' +
      this.location.name +
      '.';
    this.timestamp =
      typeof this.clockIn.sentTimestamp === 'string'
...
export class SupervisorActiveAppt extends Event {
  constructor(doc) {
    ...
    this.data = {
      ...
      action: async () => {
        window.app.snackbar.view(
          'Clocking out for ' + this.for.toUser.name.split(' ')[0] + '...'
        );
        const [e, r] = await to(Data.instantClockOut(doc.data(), doc.id));
...
export class PastAppt extends Event {
  constructor(doc) {
    ...
    this.subtitle =
      ...
      '.';
    this.timestamp =
      typeof this.clockOut.sentTimestamp === 'string'
        ...
        return new ConfirmationDialog(
          ...
          async () => {
            ...
            const [err, res] = await to(
              Data.deletePastAppt(doc.data(), doc.id)
            );
            if (err) {
              $(this.el).show();
              return window.app.snackbar.view(
            ...
            $(this.el).remove();
            window.app.schedule.refresh();
            return window.app.snackbar.view('Deleted past ' + 'appointment.');
          }
        ).view();
      },
    };
