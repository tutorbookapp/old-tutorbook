...
import { Matching, MatchingDialog } from '@tutorbook/matching';
import { SearchHeader } from '@tutorbook/search';
import { ConfirmationDialog, ViewTimeRequestDialog } from '@tutorbook/dialogs';
import { SupervisorSchedule } from '@tutorbook/schedule';
...
export class Dashboard {
  ...
  renderSelf() {
    ...
    this.main = this.render.template('dashboard', {
      ...
      subtitle:
        "We're glad you're here. Below are some " +
  ...
  viewDefaultCards(id = window.app.user.uid) {
    ...
    Object.entries({
      timestamp: ['requestsIn', 'requestsOut', 'appointments'],
      canceledTimestamp: ['canceledRequestsIn', 'canceledAppointments'],
      modifiedTimestamp: [
        'modifiedRequestsIn',
        'modifiedRequestsOut',
        'modifiedAppointments',
      ],
      approvedTimestamp: ['approvedRequestsOut'],
      rejectedTimestamp: ['rejectedRequestsOut'],
      'clockIn.sentTimestamp': ['activeAppointments'],
    }).forEach(([sortField, subcollectionsList]) =>
      subcollectionsList.forEach((subcollection) => {
        const query = window.app.db
          .collection('users')
          .doc(id)
    ...
    if (
      ...
      window.app.user.payments.type === 'Free'
    )
      this.viewCard(
  ...
  viewCard(card, list = $(this.main).find('#default')) {
    ...
    existing = $(list).find(
      '#cards [priority="' + $(card).attr('priority') + '"]'
    );
    if (existing.length) {
      // Add by timestamp
...
export class SupervisorDashboard extends Dashboard {
  renderSelf() {
    ...
    function addHorz(label) {
      $(that.main).append(that.render.divider(label)).append(that.horz.el);
    }
  ...
  viewScheduleCards() {
    ...
    if (existing.length) {
      ...
      $(this.main)
        .find('#schedule .mdc-layout-grid__inner')
  ...
  async viewRecentActivityCards() {
      ...
      switch (action.type) {
        case 'newTimeRequest':
          ...
          const confirmSummary =
            ...
            ' from ' +
            Utils.dateToTimestring(clockIn) +
            ' to ' +
            Utils.dateToTimestring(clockOut) +
            '? This action ' +
            'cannot be undone.';
    ...
    const recycler = {
      ...
      empty: (type, index) => {
        $(this.main)
          .find('#activity #cards [index="' + index + '"]')
          .remove();
    ...
    Object.entries(queries).forEach((entry) => {
      const dashboard = new ProxyDashboard(
        ...
        'Manually edit user profiles to set availability, update ' +
          'subjects, add contact information, and much more.',
        'users',
        entry[0]
      );
      this[entry[0]] = {
  ...
  viewCard(card, list) {
    const existing = $(list).find(
      '[card-id="' + $(card).attr('card-id') + '"]'
    );
    if (!$(card).attr('card-id') || !existing.length)
      return super.viewCard(card, list);
...
class QueryDashboard extends Dashboard {
  ...
  renderSections() {
    ...
    Object.entries(this.queries).forEach((section) => {
      ...
      $(this.main).append(this.render.divider(section[1].name));
      $(this.main).append(
...
class SupervisorQueryDashboard extends QueryDashboard {
  constructor() {
    ...
    const subtitle =
      "We're glad you're here. Below are some friendly " +
