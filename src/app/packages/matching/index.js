...
import { Search, SearchHeader } from '@tutorbook/search';
import { NewProfile, EditProfile } from '@tutorbook/profile';
import {
  ConfirmationDialog,
  EditRequestDialog,
  ViewRequestDialog,
...
export class Matching {
    ...
    this.matchesRecycler = {
      remove: (doc, type, user) => {
        $(this.main)
          ...
          .last()
          .remove(); // This should only ever remove one card
        switch (type) {
          case 'requestsOut':
            if (
              $(this.main).find(
                '[type="requestsOut"][user="' + doc.data().fromUser.email + '"]'
              ).length
            )
              return;
            $(this.main)
          ...
          default:
            throw new Error(
              'Invalid card type (' + type + ') passed to matchesRecycler.'
            );
        }
      },
      display: async (doc, type, user) => {
        if (this.dismissed[type] && this.dismissed[type].indexOf(doc.id) >= 0) {
          return;
        }
        switch (type) {
          case 'requestsOut':
          ...
          case 'approvedRequestsOut':
            ...
            $(card).find('.dashboard-card__title').text('Completed Match');
            break;
          case 'rejectedRequestsOut':
            var card = Card.renderRejectedRequestOutCard(doc);
            $(card).find('.dashboard-card__title').text('Rejected Match');
            break;
          default:
            throw new Error(
              'Invalid card type (' + type + ') passed to matchesRecycler.'
        ...
        $(card).attr('id', doc.id).attr('type', type).attr('user', user);
        this.viewCard(card);
      },
      empty: (type, user) => {
        $(this.main)
  ...
  async initDismissedCards() {
    this.dismissed = {};
    const cards = await window.app.db
      .collection('users')
      .doc(window.app.user.uid)
      .collection('dismissedCards')
      .get();
    cards.forEach((card) => {
  ...
  async renderPendingMatchCard(doc) {
    ...
    const subtitle =
      'From ' + request.fromUser.name + ' to ' + request.toUser.name;
    const summary =
      ...
      '. Tap to learn more and view this request.';
    const dialogs = {
    ...
    const actions = {
      ...
      cancel: () => {
        ...
        new ConfirmationDialog('Cancel Request?', summary, async () => {
          ...
          window.app.snackbar.view(
            'Canceled request to ' + request.toUser.email + '.'
          );
        }).view();
      },
      view: () => {
      ...
      options: {
        ...
        Approve: async () => {
          window.app.snackbar.view(
            'Approving request and creating ' + 'appointment...'
          );
          const [err, res] = await to(Data.approveRequest(request, doc.id));
          if (err)
          ...
          $(this.main)
            ...
            .remove();
          window.app.snackbar.view(
            'Approved request and created ' + 'appointment.'
          );
        },
  ...
  createPupil() {
    ...
    new NewProfile(
      Utils.combineMaps(Data.emptyProfile, {
        type: 'Pupil',
  ...
  addUserQuery(id) {
    ...
    this.queries[id] = [];
    ['requestsOut', 'approvedRequestsOut', 'rejectedRequestsOut'].forEach(
      (subcollection) => {
        this.queries[id].push(
          window.app.db
            .collection('users')
            .doc(id)
            ...
            .onSnapshot({
              error: (err) => {
                ...
                console.error(
                  '[ERROR] Could not get (' + id + ') ' + 'matches b/c of ',
                  err
                );
              },
              next: (snapshot) => {
                ...
                snapshot.docChanges().forEach((change) => {
                  if (change.type === 'removed') {
                    this.matchesRecycler.remove(change.doc, subcollection, id);
                  } else {
                    this.matchesRecycler.display(change.doc, subcollection, id);
                  }
                });
              },
            })
  ...
  viewCard(card) {
    ...
    var existing = $(this.main).find(
      '#cards [id="' + $(card).attr('id') + '"]'
    );
    if (existing.length) {
  ...
  static renderShortcutCard() {
    ...
    const summary =
      "With manual matching, you're able to create proxy " +
  ...
  renderCard(doc) {
    ...
    const summary =
      ...
      profile.email +
      '. Tap to view or edit ' +
      Utils.getPronoun(profile.gender) +
      ' profile.';
    var card;
    const actions = {
      hide: async () => {
        ...
        if (err) {
          window.app.snackbar.view(
            'Could not hide ' + profile.name + "'s card."
          );
          $(card).show();
...
export class MatchingDialog {
  ...
  manage() {
    ...
    const d = {
      ...
      off:
        'Showing users without appointments during the selected ' + 'timeslot.',
    };
    $(this.main)
      .find('[id="Show booked users"] .mdc-switch input')[0]
      .addEventListener('click', () => {
        this.search.filters.showBooked = !this.search.filters.showBooked;
        this.search.viewResults();
        $(this.main)
          .find('[id="Show booked users"] .mdc-list-item__secondary' + '-text')
          .text(this.search.filters.showBooked ? d.on : d.off);
      });
  }

  renderSelf() {
    // TODO: Add hover-for-more-info on tutor search results
    ...
    add(
      s('Time', this.time, Utils.getAvailabilityStrings(profile.availability))
    );
    add(this.renderSchedule());
    add(
      this.render.switch('Show booked users', {
        on: 'Showing users with appointments during the selected timeslot.',
        off:
          'Showing users without appointments during the selected ' +
          'timeslot.',
      })
    );
    addActionD(
      (profile.type === 'Tutor' ? 'Pupils' : 'Tutors') + ' for ' + this.subject,
      {
        match: () => {
    ...
    $(this.main)
      .find('.action-list-divider button')
      .last()
  ...
  renderSchedule() {
    ...
    schedule.displayHook = (doc, type) => {
      ...
      $(schedule.main)
        ...
        .remove()
        .end()
        .find('h2 [data-fir-content="title"]')
        .text(a.for.subject)
        .end()
        .find('h3')
        .text(a.time.from + ' with ' + other.name)
        .end()
        .find('#menu')
        .hide(); // TODO: Fix menu mdc-list-item styling.
    };
  ...
  results() {
    ...
    $(this.main)
      .find('.action-list-divider')
      .last()
      .find('h4 span')
      .text(
        (this.profile.type === 'Tutor' ? 'Pupils' : 'Tutors') +
          ' for ' +
          this.subject
      );
...
class MatchingSearch extends Search {
  constructor(pupil, subject, time, selectedUsers, addUser, removeUser) {
    ...
    this.filters.availability =
      !!time && time !== '' ? Utils.parseAvailabilityString(time) : {};
...
class ConfirmMatchDialog extends ConfirmationDialog {
  constructor(pupil, tutors, subject, timeString) {
    ...
    async function match(tutor) {
      ...
      const request = {
        ...
        location: {
          ...
          id:
            window.app.data.locationsByName[time.location] ||
            window.app.location.id,
        },
    ...
    async function createMatches() {
      window.app.nav.back();
      window.app.snackbar.view(
        'Creating match(es) and sending request' + '(s)...'
      );
      var errored = 0;
      await Promise.all(
        tutors.map(async (tutor) => {
          ...
          if (err) {
            window.app.snackbar.view(
              'Could not create match or send ' +
                'request to ' +
                tutor.email +
                '.'
            );
          ...
          () =>
            window.app.snackbar.view(
              errored +
                ...
                tutors.length +
                ' match(es) ' +
                'errored and should be tried again.'
            ),
          2000
        );
      window.app.snackbar.view(
        'Created match(es) and sent request(s) ' +
          'to ' +
          tutors.length +
          ' users.'
      );
