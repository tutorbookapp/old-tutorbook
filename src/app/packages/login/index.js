...
export class Login {
  ...
  renderSelf() {
    ...
    this.main = this.render.template('login', {
      ...
      paidTutor: () => {
        Utils.url(
          '/app/home?cards=' +
            window.encodeURIComponent(['setupProfile', 'setupNotifications']) +
      ...
      supervisor: () => {
        ...
        Utils.url(
          '/app/home?cards=' +
            window.encodeURIComponent(['setupNotifications']) +
    ...
    return firebase
      ...
      .catch((error) => {
        ...
        window.app.snackbar.view(
          'Could not open Google login. Reload ' + 'this page and try again.'
        );
  ...
  static async codeSignIn() {
    ...
    $(dialogEl)
      ...
      .text(
        'To ensure that you are ' +
          'an authenticated ' +
          window.app.user.type.toLowerCase() +
          ', ' +
    ...
    confirmButton.addEventListener('click', () => {
      try {
        if (codes[firebase.auth().currentUser.uid] === codeTextField.value) {
          dialog.close();
...
export class GoToRootPrompt {
  ...
  renderSelf() {
    ...
    this.main = this.render.template('login-prompt', {
      title: 'Unauthorized.',
      description:
        'Sorry, you must login with an ' +
        Utils.join(
          this.access.domains.map((d) => '@' + d),
          'or',
          false
        ) +
        ' email address to access this ' +
        this.access.name +
        ' app. ' +
...
export class WebsiteConfigMissingPrompt {
  ...
  renderSelf() {
    ...
    this.main = this.render.template('login-prompt', {
      ...
      description:
        'We could not find the website configuration for ' +
        window.location.hostname +
        '. Did you mean to go to our ' +
        'public web app?',
    ...
    $(this.main)
      ...
      .prepend(
        this.render.template('login-prompt-btn', {
          label: 'Go to public app',
          action: () => (window.location = 'https://tutorbook.app/app/'),
        })
      )
