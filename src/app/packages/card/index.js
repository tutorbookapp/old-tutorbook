...
// Users
import { EditProfile } from '@tutorbook/profile';
...
Card.renderUserCard = (doc) => {
  ...
  const card = Card.renderCard(
    p.name,
    p.grade && p.type ? p.grade + ' ' + p.type : p.type || p.grade,
    '',
    {
      options: {
            ...
            async () => {
              ...
              if (err)
                return window.app.snackbar.view(
                  'Could not ' + 'delete ' + p.name + "'s account."
                );
  ...
  return $(card)
    .addClass('mdc-layout-grid__cell--span-2')
    .find('.dashboard-card__secondary')
    .remove()
    .end()[0];
};
...
Card.renderTutorsCard = function (doc) {
  const summary =
    ...
    'dashboard.';
  const title =
    window.app.dashboard.tutors.num +
    ' ' +
  ...
  const actions = {
    ...
    backup: async () => {
      ...
      if (err)
        return window.app.snackbar.view('Could not generate PDF ' + 'backup.');
...
Card.renderPupilsCard = function (doc) {
  const summary =
    ...
    'pupils.';
  const title =
    window.app.dashboard.pupils.num +
    ' ' +
    window.app.location.name.split(' ')[0] +
    ' Pupils';
  const subtitle =
...
Card.renderMatchesCard = function (doc) {
  ...
  const summary =
    'Quia voluptatem harum temporibus. Ea culpa eum ' +
...
Card.renderSearchTutorsCard = function () {
  ...
  $(card).attr('id', 'searchTutorsCard').attr('priority', 1);
...
Card.renderSetupAvailabilityCard = function (subtitle, summary) {
  ...
  const card = Card.renderCard(
    'Set Availability',
    'Enable tutors to modify your requests',
...
Card.renderSetupProfileCard = function () {
  ...
  const summary =
    'Customize your profile to help others find, message, ' +
...
Card.renderModifiedRequestInCard = function (doc) {
  ...
  if (app.user.type === 'Supervisor') {
    ...
    var summary =
      data.modifiedBy.name.split(' ')[0] +
      ' modified ' +
      pronoun +
      ' lesson request' +
      ' to ' +
      data.for.toUser.name.split(' ')[0] +
    ...
    var summary =
      ...
      ' modified ' +
      pronoun +
      ' lesson request to you. Please ensure to address these' +
...
Card.renderCanceledRequestInCard = function (doc) {
  ...
  if (app.user.type === 'Supervisor') {
    var subtitle =
      data.canceledBy.name +
      ' canceled ' +
      pronoun +
      ' request to ' +
      data.for.toUser.name;
    var summary =
      data.canceledBy.name.split(' ')[0] +
      ' canceled ' +
      pronoun +
      ' request to ' +
      data.for.toUser.name.split(' ')[0] +
...
Card.renderApprovedRequestOutCard = function (doc) {
  ...
  const subtitle =
    app.user.type === 'Supervisor'
      ? data.approvedBy.name.split(' ')[0] +
  ...
  const summary =
    ...
    (app.user.type === 'Supervisor'
      ? ' approved a ' + 'request from ' + otherUser.name
      : ' approved the request you sent') +
          ...
          .then((doc) => {
            ...
            return window.app.db
              ...
              .then((doc) => {
                ...
                console.error(
                  '[ERROR] Could not find appt ' +
                    'document for approvedRequest:',
                  doc.id
                );
              });
          });
...
Card.renderModifiedApptCard = function (doc) {
  const data = doc.data();
  var subtitle =
    data.modifiedBy.name.split(' ')[0] + ' modified ' + 'your appointment';
  var summary =
    data.modifiedBy.name +
    ' modified your ' +
...
Card.renderCanceledApptCard = function (doc) {
  ...
  const summary =
    app.user.type === 'Supervisor'
      ? data.canceledBy.name +
        ' canceled ' +
        Utils.getPronoun(data.canceledBy.gender) +
        ' tutoring appointment with ' +
        otherUser.name +
        '. Please ' +
        'ensure to address these changes as necessary.'
      : [data.for.attendees[0].email, data.for.attendees[1].email].indexOf(
          data.canceledBy.email
        ) < 0
      ? data.canceledBy.name +
        ' canceled your ' +
        'appointment with ' +
...
Card.renderRequestInCard = function (doc) {
  ...
  var summary =
    ...
    '. Tap to learn more and setup an appointment.';
  var actions = {};
  var err;
  var res;
  var card;
  actions.reject = function () {
    ...
    new ConfirmationDialog('Reject Request?', summary, async () => {
      ...
      $(card).remove();
      window.app.snackbar.view(
        'Rejected request from ' + request.fromUser.email + '.'
      );
    }).view();
  };
...
Card.renderRequestOutCard = function (doc) {
  ...
  const summary =
    ...
    's at the ' +
    request.location.name +
    '. Tap to learn more and edit your request.';
  const actions = {};
  actions.primary = function () {
    new ViewRequestDialog(request, doc.id).view();
  };
...
Card.renderActiveApptCard = function (doc) {
  ...
  if (app.user.type === 'Supervisor') {
    var subtitle =
      'Between ' + appt.attendees[0].name + ' and ' + appt.attendees[1].name;
    var summary =
      'Tutoring session right now between ' +
    ...
    var summary =
      'Tutoring session right now with ' +
      withUser.name +
      ' for ' +
      appt.for.subject +
      ' on ' +
...
Card.renderApptCard = function (doc) {
  ...
  if (app.user.type === 'Supervisor') {
    ...
    var subtitle = 'With ' + withUser.name;
    var summary =
      'You have tutoring sessions with ' +
      withUser.name +
      ' for ' +
      appt.for.subject +
      ' on ' +
...
Card.renderNeededPaymentCard = function (doc) {
  ...
  const subtitle = 'Send $' + payment.amount + ' to ' + payment.to.name + '.';
  const summary =
    ...
    payment.appt.time.day +
    '. If you were not satisfied, do not click approve.';
  const actions = {
    deny: () => {
      return new ConfirmationDialog(
        ...
        async () => {
          ...
          window.app.snackbar.view(
            'Denied payment to ' + approvedPayment.to.email + '.'
          );
        }
      ).view();
    },
    approve: async () => {
      ...
      window.app.snackbar.view(
        'Approved payment to ' + approvedPayment.to.email + '.'
      );
    },
  };
  const card = Card.renderCard(title, subtitle, summary, actions);
...
Card.renderNeedApprovalPaymentCard = function (doc) {
  ...
  const title = 'Approve Payment';
  const subtitle =
    'Send $' + payment.amount.toFixed(2) + ' to ' + payment.to.name + '.';
  const summary =
    'Approve and send payment ($' +
...
Card.renderCard = function (title, subtitle, summary, actions) {
  ...
  if (typeof actions.options === 'object') {
    $(card)
      .find('.dashboard-card__primary-action')
      .removeClass('mdc-card__primary-action');
    if (Object.entries(actions).length < 2)
      $(card).find('.mdc-card__actions').remove();
    Object.entries(actions.options).forEach((entry) => {
      $(card)
        .find('.mdc-list')
        .append(
          window.app.render.template('card-action', {
            label: entry[0],
            action: entry[1],
          })
        );
