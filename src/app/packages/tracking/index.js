...
export default class Tracking {
  static renderShortcutCard() {
    const url =
      window.app.location.name === 'Gunn Academic Center'
        ? 'https://docs.google.com/spreadsheets/d/1NRdoDa1VDcivCFUCZLLwBsOe' +
          'Swlp89QessS4BzNLejs/edit?usp=sharing'
    ...
    const summary =
      "Manipulate and manage your tutors' service hours " +
    ...
    const actions = {
          ...
          .then((res) => {
            if (res.data.err)
              return window.app.snackbar.view(
                'Could ' + 'not update sheet. Try again in a few minutes.'
              );
      ...
      export: async () => {
        ...
        if (err)
          return window.app.snackbar.view(
            'Could not generate ' + 'service hour logs.'
          );
        window.app.snackbar.view(
          'Generated service hour logs.',
          'view',
          () => window.open(res),
          true,
          -1
