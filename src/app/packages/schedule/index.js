...
import { NewPastApptDialog } from '@tutorbook/dialogs';
...
export class Schedule {
    ...
    this.recycler = {
      ...
      display: (doc, type) => {
        try {
          ...
          console.warn(
            '[ERROR] Could not render ' + type + ' (' + doc.id + ') b/c of',
            e
          );
        }
      },
   ...
   */
  viewAppts() {
    const db = window.app.db.collection('users').doc(window.app.user.uid); // TODO: Add proxy results too
    const queries = {
      ...
      summary:
        window.app.user.type === 'Supervisor'
          ? 'View all past,' +
            ...
            ' you supervise.'
          : 'View past tutoring sessions, clock ' +
  ...
  viewAppt(listItem) {
    ...
    for (var i = 0; i < scheduleEl.children.length; i++) {
      ...
      if (time && time <= timestamp) {
        if (
          (child.previousElementSibling && // Is this a list divider?
            new Date(
              child.previousElementSibling.getAttribute('timestamp')
            ).toDateString() === timestamp.toDateString()) ||
          new Date(child.getAttribute('timestamp')).toDateString() ===
  ...
  static renderDateDivider(date) {
    ...
    const earliestDateOnDate = new Date(
      date.getFullYear(),
      date.getMonth(),
      date.getDate(),
      0,
    ...
    const templateName = window.app.onMobile
      ? 'mobile-date-list-divider'
      : 'date-list-divider';
  ...
  static getDayAndDateString(date) {
    ...
    return (
      abbr[date.getDay()] + ', ' + (date.getMonth() + 1) + '/' + date.getDate()
    );
  }
...
export class SupervisorSchedule extends Schedule {
  ...
  renderSelf() {
    ...
    this.search = new SearchHeader({
      ...
      placeholder: window.app.onMobile
        ? 'Search appointments'
        : 'Searc' + 'h appointments by subject, location, time, and more',
    ...
    $(this.main).append(
      this.render.template('fab-labeled', {
  ...
  static renderShortcutCard() {
    ...
    const summary =
      "From your schedule, you're able to view, edit, and " +
