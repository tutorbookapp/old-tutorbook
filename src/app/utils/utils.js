...
function updatePackageNames(packages, packagesDir) {
  // TODO: Get packages from packagesDir
  ...
  packages.forEach((packageDir) => {
    ...
    if (packageJson.name.indexOf('@tutorbook/') < 0) {
      ...
      packageJson.name =
        '@tutorbook/' + packageJson.name.replace('tutorbook-', '');
  ...
  console.log(
    '[INFO] Checked ' +
      packages.length +
      ' packages and updated ' +
      packageNamesChanged.length +
      " of those packages' package.json file."
  );
  console.log(
    '[INFO] Searching ' +
      packages.length +
